<main>
    <section class="pt-0">
        <div class="container">

            <!-- Filter bar START -->
            <form class="p-4 my-4 border bg-light rounded-3 z-index-9 position-relative">
                <div class="row g-3">
                    <!-- Input -->
                    <div class="col-xl-3">
                        <input class="form-control me-1" type="search" placeholder="جستجو براساس کلمه کلیدی">
                    </div>

                    <!-- Select item -->
                    <div class="col-xl-8">
                        <div class="row g-3">
                            <!-- Select items -->
                            <div class="pb-2 col-sm-6 col-md-3 pb-md-0">
                                <select class="form-select form-select-sm js-choice"
                                    aria-label=".form-select-sm example">
                                    <option value="">دسته بندی ها</option>
                                    <option>همه</option>
                                    <option>طراحی وب</option>
                                    <option>هنر</option>
                                    <option>حسابداری</option>
                                    <option>داده کاوی</option>
                                    <option>مالی</option>
                                    <option>شبکه</option>
                                    <option>عکاسی</option>
                                    <option>نوشتاری</option>
                                    <option>برنامه نویسی</option>
                                </select>
                            </div>

                            <!-- Search item -->
                            <div class="pb-2 col-sm-6 col-md-3 pb-md-0">
                                <select class="form-select form-select-sm js-choice"
                                    aria-label=".form-select-sm example">
                                    <option value="">قیمت</option>
                                    <option>همه</option>
                                    <option>رایگان</option>
                                    <option>خریدنی</option>
                                </select>
                            </div>

                            <!-- Search item -->
                            <div class="pb-2 col-sm-6 col-md-3 pb-md-0">
                                <select class="form-select form-select-sm js-choice"
                                    aria-label=".form-select-sm example">
                                    <option value="">مهارت</option>
                                    <option>همه سطح</option>
                                    <option>مبتدی</option>
                                    <option>مقدماتی</option>
                                    <option>پیشرفته</option>
                                </select>
                            </div>

                            <!-- Search item -->
                            <div class="pb-2 col-sm-6 col-md-3 pb-md-0">
                                <select class="form-select form-select-sm js-choice"
                                    aria-label=".form-select-sm example">
                                    <option value="">زبان</option>
                                    <option>انگلیسی</option>
                                    <option>فرانسه</option>
                                    <option>روسیه</option>
                                    <option>هندی</option>
                                    <option>آلمان</option>
                                    <option>اسپانیا</option>
                                </select>
                            </div>
                        </div> <!-- Row END -->
                    </div>
                    <!-- Button -->
                    <div class="col-xl-1">
                        <button type="button" class="mb-0 rounded btn btn-primary z-index-1 w-100"><i
                                class="fas fa-search"></i></button>
                    </div>
                </div> <!-- Row END -->
            </form>
            <!-- Filter bar END -->

            <div class="mt-3 row">
                <!-- Main content START -->
                <div class="col-12">

                    <!-- Course Grid START -->
                    <div class="row g-4">

                        <!-- Card item START -->
                        <div class="col-sm-6 col-lg-4 col-xl-3">
                            <div class="shadow card h-100">
                                <!-- Image -->
                                <img src="assets/images/courses/4by3/08.jpg" class="card-img-top" alt="course image">
                                <!-- Card body -->
                                <div class="pb-0 card-body">
                                    <!-- Badge and favorite -->
                                    <div class="mb-2 d-flex justify-content-between">
                                        <a href="#" class="badge bg-purple bg-opacity-10 text-purple">همه سطح</a>
                                        <a href="#" class="mb-0 h6 fw-light"><i class="far fa-heart"></i></a>
                                    </div>
                                    <!-- Title -->
                                    <h5 class="card-title fw-normal"><a href="#">آموزش Sketch</a></h5>
                                    <!-- Rating star -->
                                    <ul class="mb-0 list-inline">
                                        <li class="list-inline-item me-0 small"><i class="fas fa-star text-warning"></i>
                                        </li>
                                        <li class="list-inline-item me-0 small"><i class="fas fa-star text-warning"></i>
                                        </li>
                                        <li class="list-inline-item me-0 small"><i class="fas fa-star text-warning"></i>
                                        </li>
                                        <li class="list-inline-item me-0 small"><i class="fas fa-star text-warning"></i>
                                        </li>
                                        <li class="list-inline-item me-0 small"><i class="far fa-star text-warning"></i>
                                        </li>
                                        <li class="mb-0 list-inline-item ms-2 h6 fw-light">4.0/5.0</li>
                                    </ul>
                                </div>
                                <!-- Card footer -->
                                <div class="pt-0 pb-3 card-footer">
                                    <hr>
                                    <div class="d-flex justify-content-between">
                                        <span class="mb-0 h6 fw-light"><i
                                                class="far fa-clock text-danger me-2"></i>12دقیقه</span>
                                        <span class="mb-0 h6 fw-light"><i class="fas fa-table text-orange me-2"></i>15
                                            ویدیو</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <!-- Card item END -->

                        <!-- Card item START -->
                        <div class="col-sm-6 col-lg-4 col-xl-3">
                            <div class="shadow card h-100">
                                <!-- Image -->
                                <img src="assets/images/courses/4by3/02.jpg" class="card-img-top" alt="course image">
                                <div class="pb-0 card-body">
                                    <!-- Badge and favorite -->
                                    <div class="mb-2 d-flex justify-content-between">
                                        <a href="#" class="badge bg-success bg-opacity-10 text-success">مبتدی</a>
                                        <a href="#" class="text-danger"><i class="fas fa-heart"></i></a>
                                    </div>
                                    <!-- Title -->
                                    <h5 class="card-title fw-normal"><a href="#">آموزش Photoshop</a></h5>
                                    <!-- Rating star -->
                                    <ul class="mb-0 list-inline">
                                        <li class="list-inline-item me-0 small"><i class="fas fa-star text-warning"></i>
                                        </li>
                                        <li class="list-inline-item me-0 small"><i class="fas fa-star text-warning"></i>
                                        </li>
                                        <li class="list-inline-item me-0 small"><i class="fas fa-star text-warning"></i>
                                        </li>
                                        <li class="list-inline-item me-0 small"><i class="fas fa-star text-warning"></i>
                                        </li>
                                        <li class="list-inline-item me-0 small"><i
                                                class="fas fa-star-half-alt text-warning"></i></li>
                                        <li class="mb-0 list-inline-item ms-2 h6 fw-light">4.5/5.0</li>
                                    </ul>
                                </div>
                                <!-- Card footer -->
                                <div class="pt-0 pb-3 card-footer">
                                    <hr>
                                    <div class="d-flex justify-content-between">
                                        <span class="mb-0 h6 fw-light"><i
                                                class="far fa-clock text-danger me-2"></i>9ساعت</span>
                                        <span class="mb-0 h6 fw-light"><i class="fas fa-table text-orange me-2"></i>65
                                            ویدیو</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <!-- Card item END -->

                        <!-- Card item START -->
                        <div class="col-sm-6 col-lg-4 col-xl-3">
                            <div class="shadow card h-100">
                                <!-- Image -->
                                <img src="assets/images/courses/4by3/03.jpg" class="card-img-top" alt="course image">
                                <div class="pb-0 card-body">
                                    <!-- Badge and favorite -->
                                    <div class="mb-2 d-flex justify-content-between">
                                        <a href="#"
                                            class="badge bg-success bg-opacity-10 text-success">مبتدی</a>
                                        <a href="#" class="mb-0 h6 fw-light"><i class="far fa-heart"></i></a>
                                    </div>
                                    <!-- Title -->
                                    <h5 class="card-title fw-normal"><a href="#">آموزش Figma</a></h5>
                                    <!-- Rating star -->
                                    <ul class="mb-0 list-inline">
                                        <li class="list-inline-item me-0 small"><i
                                                class="fas fa-star text-warning"></i></li>
                                        <li class="list-inline-item me-0 small"><i
                                                class="fas fa-star text-warning"></i></li>
                                        <li class="list-inline-item me-0 small"><i
                                                class="fas fa-star text-warning"></i></li>
                                        <li class="list-inline-item me-0 small"><i
                                                class="fas fa-star text-warning"></i></li>
                                        <li class="list-inline-item me-0 small"><i
                                                class="fas fa-star-half-alt text-warning"></i></li>
                                        <li class="mb-0 list-inline-item ms-2 h6 fw-light">4.5/5.0</li>
                                    </ul>
                                </div>
                                <!-- Card footer -->
                                <div class="pt-0 pb-3 card-footer">
                                    <hr>
                                    <div class="d-flex justify-content-between">
                                        <span class="mb-0 h6 fw-light"><i
                                                class="far fa-clock text-danger me-2"></i>5دقیقه</span>
                                        <span class="mb-0 h6 fw-light"><i class="fas fa-table text-orange me-2"></i>32
                                            ویدیو</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <!-- Card item END -->

                        <!-- Card item START -->
                        <div class="col-sm-6 col-lg-4 col-xl-3">
                            <div class="shadow card h-100">
                                <!-- Image -->
                                <img src="assets/images/courses/4by3/07.jpg" class="card-img-top" alt="course image">
                                <div class="pb-0 card-body">
                                    <!-- Badge and favorite -->
                                    <div class="mb-2 d-flex justify-content-between">
                                        <a href="#"
                                            class="badge bg-success bg-opacity-10 text-success">مبتدی</a>
                                        <a href="#" class="text-danger"><i class="fas fa-heart"></i></a>
                                    </div>
                                    <!-- Title -->
                                    <h5 class="card-title fw-normal"><a href="#">آموزش React-Native</a></h5>
                                    <!-- Rating star -->
                                    <ul class="mb-0 list-inline">
                                        <li class="list-inline-item me-0 small"><i
                                                class="fas fa-star text-warning"></i></li>
                                        <li class="list-inline-item me-0 small"><i
                                                class="fas fa-star text-warning"></i></li>
                                        <li class="list-inline-item me-0 small"><i
                                                class="fas fa-star text-warning"></i></li>
                                        <li class="list-inline-item me-0 small"><i
                                                class="fas fa-star text-warning"></i></li>
                                        <li class="list-inline-item me-0 small"><i
                                                class="far fa-star text-warning"></i></li>
                                        <li class="mb-0 list-inline-item ms-2 h6 fw-light">4.0/5.0</li>
                                    </ul>
                                </div>
                                <!-- Card footer -->
                                <div class="pt-0 pb-3 card-footer">
                                    <hr>
                                    <div class="d-flex justify-content-between">
                                        <span class="mb-0 h6 fw-light"><i
                                                class="far fa-clock text-danger me-2"></i>18دقیقه</span>
                                        <span class="mb-0 h6 fw-light"><i class="fas fa-table text-orange me-2"></i>99
                                            ویدیو</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <!-- Card item END -->

                        <!-- Card item START -->
                        <div class="col-sm-6 col-lg-4 col-xl-3">
                            <div class="shadow card h-100">
                                <!-- Image -->
                                <img src="assets/images/courses/4by3/11.jpg" class="card-img-top" alt="course image">
                                <div class="pb-0 card-body">
                                    <!-- Badge and favorite -->
                                    <div class="mb-2 d-flex justify-content-between">
                                        <a href="#" class="badge bg-purple bg-opacity-10 text-purple">همه
                                            سطح</a>
                                        <a href="#" class="text-danger"><i class="fas fa-heart"></i></a>
                                    </div>
                                    <!-- Title -->
                                    <h5 class="card-title fw-normal"><a href="#">آموزش HTML</a></h5>
                                    <!-- Rating star -->
                                    <ul class="mb-0 list-inline">
                                        <li class="list-inline-item me-0 small"><i
                                                class="fas fa-star text-warning"></i></li>
                                        <li class="list-inline-item me-0 small"><i
                                                class="fas fa-star text-warning"></i></li>
                                        <li class="list-inline-item me-0 small"><i
                                                class="fas fa-star text-warning"></i></li>
                                        <li class="list-inline-item me-0 small"><i
                                                class="fas fa-star text-warning"></i></li>
                                        <li class="list-inline-item me-0 small"><i
                                                class="far fa-star text-warning"></i></li>
                                        <li class="mb-0 list-inline-item ms-2 h6 fw-light">4.0/5.0</li>
                                    </ul>
                                </div>
                                <!-- Card footer -->
                                <div class="pt-0 pb-3 card-footer">
                                    <hr>
                                    <div class="d-flex justify-content-between">
                                        <span class="mb-0 h6 fw-light"><i
                                                class="far fa-clock text-danger me-2"></i>15دقیقه</span>
                                        <span class="mb-0 h6 fw-light"><i class="fas fa-table text-orange me-2"></i>68
                                            ویدیو</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <!-- Card item END -->

                        <!-- Card item START -->
                        <div class="col-sm-6 col-lg-4 col-xl-3">
                            <div class="shadow card h-100">
                                <!-- Image -->
                                <img src="assets/images/courses/4by3/12.jpg" class="card-img-top" alt="course image">
                                <div class="pb-0 card-body">
                                    <!-- Badge and favorite -->
                                    <div class="mb-2 d-flex justify-content-between">
                                        <a href="#"
                                            class="badge bg-success bg-opacity-10 text-success">مبتدی</a>
                                        <a href="#" class="mb-0 h6 fw-light"><i class="far fa-heart"></i></a>
                                    </div>
                                    <!-- Title -->
                                    <h5 class="card-title fw-normal"><a href="#">آموزش CSS</a></h5>
                                    <!-- Rating star -->
                                    <ul class="mb-0 list-inline">
                                        <li class="list-inline-item me-0 small"><i
                                                class="fas fa-star text-warning"></i></li>
                                        <li class="list-inline-item me-0 small"><i
                                                class="fas fa-star text-warning"></i></li>
                                        <li class="list-inline-item me-0 small"><i
                                                class="fas fa-star text-warning"></i></li>
                                        <li class="list-inline-item me-0 small"><i
                                                class="fas fa-star text-warning"></i></li>
                                        <li class="list-inline-item me-0 small"><i
                                                class="fas fa-star-half-alt text-warning"></i></li>
                                        <li class="mb-0 list-inline-item ms-2 h6 fw-light">4.5/5.0</li>
                                    </ul>
                                </div>
                                <!-- Card footer -->
                                <div class="pt-0 pb-3 card-footer">
                                    <hr>
                                    <div class="d-flex justify-content-between">
                                        <span class="mb-0 h6 fw-light"><i
                                                class="far fa-clock text-danger me-2"></i>36دقیقه</span>
                                        <span class="mb-0 h6 fw-light"><i class="fas fa-table text-orange me-2"></i>72
                                            ویدیو</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <!-- Card item END -->

                        <!-- Card item START -->
                        <div class="col-sm-6 col-lg-4 col-xl-3">
                            <div class="shadow card h-100">
                                <!-- Image -->
                                <img src="assets/images/courses/4by3/05.jpg" class="card-img-top" alt="course image">
                                <div class="pb-0 card-body">
                                    <!-- Badge and favorite -->
                                    <div class="mb-2 d-flex justify-content-between">
                                        <a href="#"
                                            class="badge bg-success bg-opacity-10 text-success">مبتدی</a>
                                        <a href="#" class="mb-0 h6 fw-light"><i class="far fa-heart"></i></a>
                                    </div>
                                    <!-- Title -->
                                    <h5 class="card-title fw-normal"><a href="#">آموزش Python</a></h5>
                                    <!-- Rating star -->
                                    <ul class="mb-0 list-inline">
                                        <li class="list-inline-item me-0 small"><i
                                                class="fas fa-star text-warning"></i></li>
                                        <li class="list-inline-item me-0 small"><i
                                                class="fas fa-star text-warning"></i></li>
                                        <li class="list-inline-item me-0 small"><i
                                                class="fas fa-star text-warning"></i></li>
                                        <li class="list-inline-item me-0 small"><i
                                                class="fas fa-star text-warning"></i></li>
                                        <li class="list-inline-item me-0 small"><i
                                                class="fas fa-star-half-alt text-warning"></i></li>
                                        <li class="mb-0 list-inline-item ms-2 h6 fw-light">4.5/5.0</li>
                                    </ul>
                                </div>
                                <!-- Card footer -->
                                <div class="pt-0 pb-3 card-footer">
                                    <hr>
                                    <div class="d-flex justify-content-between">
                                        <span class="mb-0 h6 fw-light"><i
                                                class="far fa-clock text-danger me-2"></i>10ساعت</span>
                                        <span class="mb-0 h6 fw-light"><i class="fas fa-table text-orange me-2"></i>26
                                            ویدیو</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <!-- Card item END -->

                        <!-- Card item START -->
                        <div class="col-sm-6 col-lg-4 col-xl-3">
                            <div class="shadow card h-100">
                                <!-- Image -->
                                <img src="assets/images/courses/4by3/06.jpg" class="card-img-top" alt="course image">
                                <div class="pb-0 card-body">
                                    <!-- Badge and favorite -->
                                    <div class="mb-2 d-flex justify-content-between">
                                        <a href="#" class="badge bg-info bg-opacity-10 text-info">مقدماتی</a>
                                        <a href="#" class="mb-0 h6 fw-light"><i class="far fa-heart"></i></a>
                                    </div>
                                    <!-- Title -->
                                    <h5 class="card-title fw-normal"><a href="#">آموزش Angular</a></h5>
                                    <!-- Rating star -->
                                    <ul class="mb-0 list-inline">
                                        <li class="list-inline-item me-0 small"><i
                                                class="fas fa-star text-warning"></i></li>
                                        <li class="list-inline-item me-0 small"><i
                                                class="fas fa-star text-warning"></i></li>
                                        <li class="list-inline-item me-0 small"><i
                                                class="fas fa-star text-warning"></i></li>
                                        <li class="list-inline-item me-0 small"><i
                                                class="fas fa-star text-warning"></i></li>
                                        <li class="list-inline-item me-0 small"><i
                                                class="fas fa-star-half-alt text-warning"></i></li>
                                        <li class="mb-0 list-inline-item ms-2 h6 fw-light">4.5/5.0</li>
                                    </ul>
                                </div>
                                <!-- Card footer -->
                                <div class="pt-0 pb-3 card-footer">
                                    <hr>
                                    <div class="d-flex justify-content-between">
                                        <span class="mb-0 h6 fw-light"><i
                                                class="far fa-clock text-danger me-2"></i>9ساعت</span>
                                        <span class="mb-0 h6 fw-light"><i class="fas fa-table text-orange me-2"></i>42
                                            ویدیو</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <!-- Card item END -->

                        <!-- Card item START -->
                        <div class="col-sm-6 col-lg-4 col-xl-3">
                            <div class="shadow card h-100">
                                <!-- Image -->
                                <img src="assets/images/courses/4by3/10.jpg" class="card-img-top" alt="course image">
                                <div class="pb-0 card-body">
                                    <!-- Badge and favorite -->
                                    <div class="mb-2 d-flex justify-content-between">
                                        <a href="#" class="badge bg-info bg-opacity-10 text-info">مقدماتی</a>
                                        <a href="#" class="text-danger"><i class="fas fa-heart"></i></a>
                                    </div>
                                    <!-- Title -->
                                    <h5 class="card-title fw-normal"><a href="#">آموزش Bootstrap</a></h5>
                                    <!-- Rating star -->
                                    <ul class="mb-0 list-inline">
                                        <li class="list-inline-item me-0 small"><i
                                                class="fas fa-star text-warning"></i></li>
                                        <li class="list-inline-item me-0 small"><i
                                                class="fas fa-star text-warning"></i></li>
                                        <li class="list-inline-item me-0 small"><i
                                                class="fas fa-star text-warning"></i></li>
                                        <li class="list-inline-item me-0 small"><i
                                                class="fas fa-star text-warning"></i></li>
                                        <li class="list-inline-item me-0 small"><i
                                                class="fas fa-star-half-alt text-warning"></i></li>
                                        <li class="mb-0 list-inline-item ms-2 h6 fw-light">4.5/5.0</li>
                                    </ul>
                                </div>
                                <!-- Card footer -->
                                <div class="pt-0 pb-3 card-footer">
                                    <hr>
                                    <div class="d-flex justify-content-between">
                                        <span class="mb-0 h6 fw-light"><i
                                                class="far fa-clock text-danger me-2"></i>25دقیقه</span>
                                        <span class="mb-0 h6 fw-light"><i class="fas fa-table text-orange me-2"></i>38
                                            ویدیو</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <!-- Card item END -->

                        <!-- Card item START -->
                        <div class="col-sm-6 col-lg-4 col-xl-3">
                            <div class="shadow card h-100">
                                <!-- Image -->
                                <img src="assets/images/courses/4by3/13.jpg" class="card-img-top" alt="course image">
                                <div class="pb-0 card-body">
                                    <!-- Badge and favorite -->
                                    <div class="mb-2 d-flex justify-content-between">
                                        <a href="#"
                                            class="badge bg-success bg-opacity-10 text-success">مبتدی</a>
                                        <a href="#" class="mb-0 h6 fw-light"><i class="far fa-heart"></i></a>
                                    </div>
                                    <!-- Title -->
                                    <h5 class="card-title fw-normal"><a href="#">آموزش PHP</a></h5>
                                    <!-- Rating star -->
                                    <ul class="mb-0 list-inline">
                                        <li class="list-inline-item me-0 small"><i
                                                class="fas fa-star text-warning"></i></li>
                                        <li class="list-inline-item me-0 small"><i
                                                class="fas fa-star text-warning"></i></li>
                                        <li class="list-inline-item me-0 small"><i
                                                class="fas fa-star text-warning"></i></li>
                                        <li class="list-inline-item me-0 small"><i
                                                class="fas fa-star text-warning"></i></li>
                                        <li class="list-inline-item me-0 small"><i
                                                class="far fa-star text-warning"></i></li>
                                        <li class="mb-0 list-inline-item ms-2 h6 fw-light">4.0/5.0</li>
                                    </ul>
                                </div>
                                <!-- Card footer -->
                                <div class="pt-0 pb-3 card-footer">
                                    <hr>
                                    <div class="d-flex justify-content-between">
                                        <span class="mb-0 h6 fw-light"><i
                                                class="far fa-clock text-danger me-2"></i>21ساعت</span>
                                        <span class="mb-0 h6 fw-light"><i class="fas fa-table text-orange me-2"></i>30
                                            ویدیو</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <!-- Card item END -->

                        <!-- Card item START -->
                        <div class="col-sm-6 col-lg-4 col-xl-3">
                            <div class="shadow card h-100">
                                <!-- Image -->
                                <img src="assets/images/courses/4by3/01.jpg" class="card-img-top" alt="course image">
                                <div class="pb-0 card-body">
                                    <!-- Badge and favorite -->
                                    <div class="mb-2 d-flex justify-content-between">
                                        <a href="#"
                                            class="badge bg-success bg-opacity-10 text-success">مبتدی</a>
                                        <a href="#" class="text-danger"><i class="fas fa-heart"></i></a>
                                    </div>
                                    <!-- Title -->
                                    <h5 class="card-title fw-normal"><a href="#">آموزش Laravel</a></h5>
                                    <!-- Rating star -->
                                    <ul class="mb-0 list-inline">
                                        <li class="list-inline-item me-0 small"><i
                                                class="fas fa-star text-warning"></i></li>
                                        <li class="list-inline-item me-0 small"><i
                                                class="fas fa-star text-warning"></i></li>
                                        <li class="list-inline-item me-0 small"><i
                                                class="fas fa-star text-warning"></i></li>
                                        <li class="list-inline-item me-0 small"><i
                                                class="fas fa-star text-warning"></i></li>
                                        <li class="list-inline-item me-0 small"><i
                                                class="fas fa-star-half-alt text-warning"></i></li>
                                        <li class="mb-0 list-inline-item ms-2 h6 fw-light">4.5/5.0</li>
                                    </ul>
                                </div>
                                <!-- Card footer -->
                                <div class="pt-0 pb-3 card-footer">
                                    <hr>
                                    <div class="d-flex justify-content-between">
                                        <span class="mb-0 h6 fw-light"><i
                                                class="far fa-clock text-danger me-2"></i>6ساعت</span>
                                        <span class="mb-0 h6 fw-light"><i class="fas fa-table text-orange me-2"></i>82
                                            ویدیو</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <!-- Card item END -->

                        <!-- Card item START -->
                        <div class="col-sm-6 col-lg-4 col-xl-3">
                            <div class="shadow card h-100">
                                <!-- Image -->
                                <img src="assets/images/courses/4by3/04.jpg" class="card-img-top" alt="course image">
                                <div class="pb-0 card-body">
                                    <!-- Badge and favorite -->
                                    <div class="mb-2 d-flex justify-content-between">
                                        <a href="#" class="badge bg-purple bg-opacity-10 text-purple">همه
                                            سطح</a>
                                        <a href="#" class="text-danger"><i class="fas fa-heart"></i></a>
                                    </div>
                                    <!-- Title -->
                                    <h5 class="card-title fw-normal"><a href="#">آموزش Invision</a></h5>
                                    <!-- Rating star -->
                                    <ul class="mb-0 list-inline">
                                        <li class="list-inline-item me-0 small"><i
                                                class="fas fa-star text-warning"></i></li>
                                        <li class="list-inline-item me-0 small"><i
                                                class="fas fa-star text-warning"></i></li>
                                        <li class="list-inline-item me-0 small"><i
                                                class="fas fa-star text-warning"></i></li>
                                        <li class="list-inline-item me-0 small"><i
                                                class="fas fa-star-half-alt text-warning"></i></li>
                                        <li class="list-inline-item me-0 small"><i
                                                class="far fa-star text-warning"></i></li>
                                        <li class="mb-0 list-inline-item ms-2 h6 fw-light">3.5/5.0</li>
                                    </ul>
                                </div>
                                <!-- Card footer -->
                                <div class="pt-0 pb-3 card-footer">
                                    <hr>
                                    <div class="mt-2 d-flex justify-content-between">
                                        <span class="mb-0 h6 fw-light"><i
                                                class="far fa-clock text-danger me-2"></i>6ساعت</span>
                                        <span class="mb-0 h6 fw-light"><i class="fas fa-table text-orange me-2"></i>82
                                            ویدیو</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <!-- Card item END -->

                    </div>
                    <!-- Course Grid END -->

                    <!-- Pagination START -->
                    <div class="col-12">
                        <nav class="mt-4 d-flex justify-content-center" aria-label="navigation">
                            <ul class="mb-0 rounded pagination pagination-primary-soft d-inline-block d-md-flex">
                                <li class="mb-0 page-item"><a class="page-link" href="#" tabindex="-1"><i
                                            class="fas fa-angle-double-right"></i></a></li>
                                <li class="mb-0 page-item"><a class="page-link" href="#">1</a></li>
                                <li class="mb-0 page-item active"><a class="page-link" href="#">2</a></li>
                                <li class="mb-0 page-item"><a class="page-link" href="#">..</a></li>
                                <li class="mb-0 page-item"><a class="page-link" href="#">6</a></li>
                                <li class="mb-0 page-item"><a class="page-link" href="#"><i
                                            class="fas fa-angle-double-left"></i></a></li>
                            </ul>
                        </nav>
                    </div>
                    <!-- Pagination END -->
                </div>
                <!-- Main content END -->
            </div><!-- Row END -->
        </div>
    </section>
    <!-- =======================
 Page content END -->

    <!-- =======================
 Newsletter START -->
    <section class="pt-0">
        <div class="container overflow-hidden position-relative">
            <!-- SVG decoration -->
            <figure class="position-absolute top-50 start-50 translate-middle ms-3">
                <svg>
                    <path class="fill-white opacity-2"
                        d="m496 22.999c0 10.493-8.506 18.999-18.999 18.999s-19-8.506-19-18.999 8.507-18.999 19-18.999 18.999 8.506 18.999 18.999z" />
                    <path class="fill-white opacity-2"
                        d="m775 102.5c0 5.799-4.701 10.5-10.5 10.5-5.798 0-10.499-4.701-10.499-10.5 0-5.798 4.701-10.499 10.499-10.499 5.799 0 10.5 4.701 10.5 10.499z" />
                    <path class="fill-white opacity-2"
                        d="m192 102c0 6.626-5.373 11.999-12 11.999s-11.999-5.373-11.999-11.999c0-6.628 5.372-12 11.999-12s12 5.372 12 12z" />
                    <path class="fill-white opacity-2"
                        d="m20.499 10.25c0 5.66-4.589 10.249-10.25 10.249-5.66 0-10.249-4.589-10.249-10.249-0-5.661 4.589-10.25 10.249-10.25 5.661-0 10.25 4.589 10.25 10.25z" />
                </svg>
            </figure>
            <!-- Svg decoration -->
            <figure class="bottom-0 mb-5 position-absolute end-0 d-none d-sm-block">
                <svg class="rotate-130" width="258.7px" height="86.9px" viewBox="0 0 258.7 86.9">
                    <path stroke="white" fill="none" stroke-width="2"
                        d="M0,7.2c16,0,16,25.5,31.9,25.5c16,0,16-25.5,31.9-25.5c16,0,16,25.5,31.9,25.5c16,0,16-25.5,31.9-25.5 c16,0,16,25.5,31.9,25.5c16,0,16-25.5,31.9-25.5c16,0,16,25.5,31.9,25.5s16-25.5,31.9-25.5" />
                    <path stroke="white" fill="none" stroke-width="2"
                        d="M0,57c16,0,16,25.5,31.9,25.5c16,0,16-25.5,31.9-25.5c16,0,16,25.5,31.9,25.5c16,0,16-25.5,31.9-25.5 c16,0,16,25.5,31.9,25.5c16,0,16-25.5,31.9-25.5c16,0,16,25.5,31.9,25.5s16-25.5,31.9-25.5" />
                </svg>
            </figure>

            <div class="p-3 bg-grad-blue p-sm-5 rounded-3">
                <div class="row justify-content-center position-relative">
                    <!-- SVG decoration -->
                    <figure class="position-absolute top-50 start-0 translate-middle-y">
                        <svg width="141px" height="141px">
                            <path class="fill-white opacity-1"
                                d="M140.520,70.258 C140.520,109.064 109.062,140.519 70.258,140.519 C31.454,140.519 -0.004,109.064 -0.004,70.258 C-0.004,31.455 31.454,-0.003 70.258,-0.003 C109.062,-0.003 140.520,31.455 140.520,70.258 Z" />
                        </svg>
                    </figure>
                    <!-- Newsletter -->
                    <div class="my-2 col-12 position-relative my-sm-3">
                        <div class="row align-items-center">
                            <!-- Title -->
                            <div class="col-lg-6">
                                <h3 class="mb-3 text-white mb-lg-0 ff-vb">دریافت جدیدترین به‌روزرسانی‌های دوره، در
                                    خبرنامه ما</h3>
                            </div>
                            <!-- Input item -->
                            <div class="col-lg-6 text-lg-end">
                                <form class="p-2 rounded bg-body">
                                    <div class="input-group">
                                        <input class="border-0 form-control me-1" type="email" placeholder="ایمیل">
                                        <button type="button" class="mb-0 rounded btn btn-dark">عضویت</button>
                                    </div>
                                </form>
                            </div>
                        </div>
                    </div>
                </div> <!-- Row END -->
            </div>
        </div>
    </section>
    <!-- =======================
 Newsletter END -->

</main>
